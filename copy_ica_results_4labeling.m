...
for i = 1:length(rat_list)
    scan_list = dir(fullfile(data_dir, rat_list(i).name, 'rfmri_intermediate', 'mr*'));
    for j = 1:length(scan_list)
        copyfile(fullfile(scan_list(j).folder, [scan_list(j).name(3:4), '.gift_ica']),...
            fullfile(ica_dir, [rat_list(i).name, '_', scan_list(j).name(3:4)]));
    end
end
...
for i = 1:length(list)
    ...
    for j = 1:50
        ...
        fid = fopen(fullfile(data_dir, list(i).name(1:6), 'rfmri_intermediate', ...
            [list(i).name(8:9), '_despiked.json']), 'r');
        s = fread(fid);
        a = jsondecode(char(s)');
